...
run_thetam<-function(i,data, id, periodsToForecast, start_date, run_type){
   ...
   ts=na.remove(ts)
    ...
    hForecast <- forecast(hm1, h = periodsToForecast, level=.95) 
        ...
        fct1<-data.frame(ds=zoo::as.Date(zoo::yearmon((time(hForecast$thetam$fitted))),frac=0),yhat=as.numeric(hForecast$thetam$fitted))%>%pivot_wider(names_from=ds,values_from=yhat)

        fct2<-data.frame(ds=zoo::as.Date(zoo::yearmon((time(hForecast$x))),frac=0),yhat=as.numeric(hForecast$x))%>%pivot_wider(names_from=ds,values_from=yhat)

        fct3<-data.frame(ds=zoo::as.Date(zoo::yearmon((time(hForecast$thetam$mean))),frac=0),yhat=as.numeric(hForecast$thetam$mean))%>%pivot_wider(names_from=ds,values_from=yhat)
    ...
    fct$rmse=forecast::accuracy(hm1$thetam)[2]
    fct$mape=forecast::accuracy(hm1$thetam)[5]

    fct$tag=thisTag[[1]]

    fct[fct<0] <- 0

    fct$confidence=mean((hForecast$thetam$upper-hForecast$thetam$mean)/hForecast$thetam$mean)
...
write.csv(fct_output6, paste0(wd, '/selected_models/fct_output6.csv'))
